...
package gov.sandia.phoenix.noise

import scala.math._

/**
 * An implementation of Ken Perlin's simplex noise functions based on http://staffwww.itn.liu.se/~stegu/simplexnoise/SimplexNoise.java.
 * TODO: Make things more canonically functional.
 */
object SimplexNoise {
  private final val grad3 = Array(Grad(1,1,0),Grad(-1,1,0),Grad(1,-1,0),Grad(-1,-1,0),
    Grad(1,0,1),Grad(-1,0,1),Grad(1,0,-1),Grad(-1,0,-1),
    Grad(0,1,1),Grad(0,-1,1),Grad(0,1,-1),Grad(0,-1,-1))

  private final val grad4 = Array(Grad(0,1,1,1),Grad(0,1,1,-1),Grad(0,1,-1,1),Grad(0,1,-1,-1),
  ...
  private final val p = Array[Short](151,160,137,91,90,15,
  ...
  private final val perm = Array.tabulate[Short](512){ i => p(i & 255) }
  private final val permMod12 = Array.tabulate[Short](512){ i => (perm(i) % 12).toShort }

  // Skewing and unskewing factors for 2, 3, and 4 dimensions
  private final val F2 = 0.5*(Math.sqrt(3.0)-1.0)
  private final val G2 = (3.0-sqrt(3.0))/6.0
  private final val F3 = 1.0/3.0
  private final val G3 = 1.0/6.0
  private final val F4 = (sqrt(5.0)-1.0)/4.0
  private final val G4 = (5.0-sqrt(5.0))/20.0

  // This method is a *lot* faster than using (int)Math.floor(x)
  private final def fastfloor(x : Double) = {
    val xi = x.toInt
    if(x < xi) xi - 1 else xi
  }

  private final def dot(g : Grad, x : Double, y : Double) =  g.x*x + g.y*y

  private final def dot(g : Grad, x : Double, y : Double, z : Double) = g.x*x + g.y*y + g.z*z

  private final def dot(g : Grad, x : Double, y : Double, z : Double, w : Double) =  g.x*x + g.y*y + g.z*z + g.w*w

  // 2D simplex noise
  final def noise(xin : Double, yin : Double) = {
    var n0 = 0.0
    var n1 = 0.0
    var n2 = 0.0 // Noise contributions from the three corners
  ...
  // 3D simplex noise
  final def noise(xin : Double, yin : Double, zin : Double) = {
    var n0 = 0.0
    var n1 = 0.0
    var n2 = 0.0
    var n3 = 0.0; // Noise contributions from the four corners
    // Skew the input space to determine which simplex cell we're in
  ...
  final def noise(x : Double, y : Double, z : Double, w : Double) = {
    var n0 = 0.0
    var n1 = 0.0
    var n2 = 0.0
    var n3 = 0.0
    ...
    var rankz = 0
    var rankw = 0
    if(x0 > y0) rankx = rankx + 1 else ranky = ranky + 1
    if(x0 > z0) rankx = rankx + 1 else rankz = rankz + 1
    if(x0 > w0) rankx = rankx + 1 else rankw = rankw + 1
    if(y0 > z0) ranky = ranky + 1 else rankz = rankz + 1
    if(y0 > w0) ranky = ranky + 1  else rankw = rankw + 1
    if(z0 > w0) rankz = rankz + 1 else rankw = rankw + 1
    var i1 = 0
    var j1 = 0
    var k1 = 0
    ...
    var j3 = 0
    var k3 = 0
    var l3 = 0 // The valeger offsets for the fourth simplex corner
...
//The original author asserts this is faster than array access. For real?
case class Grad(x : Double, y : Double, z : Double, w : Double = 0.0)

object SimplexNoiseTester extends App {
  println("SimplexNoiseTester")
  println(SimplexNoise.noise(45.0, 465.0))
}